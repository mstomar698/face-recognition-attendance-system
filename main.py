...
def TakeImages():
    ...
    if exists:
       for row in open("StudentDetails\StudentDetails.csv"):
            serial = serial + 1
    else:
...
def TrackImages():
    ...
    if exists1:
        df = pandas.read_csv("StudentDetails\StudentDetails.csv")
    else:
        messagebox._show(title='Details Missing', message='Students details are missing, please check!')
        cam.release()
        cv2.destroyAllWindows()
        window.destroy()
    while True:
...
datef = tkinter.Label(dateBox, text = day+"-"+mont[month]+"-"+year+"  |  ", fg="orange",bg="#8f4155" ,width=55 ,height=1,font=('times', 22, ' bold '))
...
# ENTRY FIELDS
rollText = tkinter.Label(frame2, text="Enter your Roll No",width=30  ,fg="white"  ,bg="#FFCE30" ,font=('times', 18, ' bold ') )
